...
The pull request pipeline is about 330 step definitions. A program in this repo, `buildkite/pipeline_generator/` (the **generator**), reads the step yaml and decides which to emit, from two hand-written inputs:
...
**Broad entries run tests that cannot be affected.** Ninety steps across the three pipelines declare a bare `vllm/`, so they run on nearly every change.

Both follow from the same thing: the mapping from code to tests is maintained by hand, at a size nobody can review against the import structure, and the generator that reads it lives in a different repository from the lists themselves.
...
**None of this is wired up yet.** The tool prints a job list and nothing reads it: no Buildkite step and no workflow calls it, and turning it on is a separate piece of work. This section is the design for that, and the two gaps below are what it still has to clear.
...
**Emitted names are not verified against the generator.** The safeguard above only fires when the tool can produce no name at all. A name it produces but spells wrong passes. The generator then rejects the name and the whole upload fails. Both of the old causes are closed: the key derivation is now the generator's own imported function rather than a transcription, and since ci-infra #520 a pinned build accepts `amd-` mirror keys, which is the spelling the tool emits.
...
**Drift tests.** 61 tests read the real vLLM tree and the generator beside us, and fail when something moves. Each failure message is written for whoever trips it, who has probably not seen this tool: what moved, what it costs selection, and the exact edit.

**Detection floors.** Every check asks whether anything unknown is present, and that question reports green when the detector has stopped detecting, because zero findings and zero capability look identical from outside. So checks assert their own output volume. The comparison against the generator fails if it verified fewer than a thousand cases, so a parser that stops finding things fails loudly rather than reporting a clean repository.
...
Python 3.12 or newer, depending on `pyyaml`, `regex`, and the `pipeline_generator` package beside it in this repo.

```bash
ci-select --repo /path/to/vllm --diff origin/main...HEAD             # both inputs, the real answer
