...
def matches_source_dependency(dep: str, diff_file: str) -> bool:
    ...
    directory prefix at a `/` boundary (no globbing). Its own function."""
    return _matches_source_dependency(dep, diff_file)
...
def deps_match(deps: list[str] | None, paths: list[str]) -> bool:
    """Does any changed path satisfy this step's declarations? The generator's
    own whole-diff answer, imported."""
    return _source_file_dependencies_match(deps, paths)
