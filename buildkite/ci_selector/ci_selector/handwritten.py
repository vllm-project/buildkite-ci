...
Two kinds live here: what this repo's pipeline generator does that it does not
expose as a name, and which dynamic imports leave the vLLM repo. Neither is
something the tool can read for itself. Facts about vLLM's own tree do NOT
belong here, because the tool already reads that tree; they sit beside the code
that reads them, and stay central only when two modules share one.
...
# The pipeline generator, a sibling package in this repo. It publishes flat
# top-level modules, so these imports are unqualified.
import amd as _amd
from amd import AMD_ARTIFACT_STEP
from global_config import ONLY_STEP_KEYS_ENV_VAR
...
# =========================================================================
# 4. WHAT CI-INFRA'S GENERATOR DOES
# What ci-infra's pipeline generator does. Anything it exposes as a name is
# imported straight from it; what is written out below has no name to import.
# Update when: it changes.
# Guard: the generator is imported, so anything it exposes as a name is taken
# straight from it and cannot drift. What is left here is what the generator
# writes as a bare literal inside a function body, with no name to import; a
# drift test reads those out of the live source. The two field sets have a
# second guard as well: they fail when vLLM's yaml uses a name we do not model.
# Everything below ONLY_STEP_KEYS_ENV is a vLLM-tree or own-policy fact, not a
# generator copy, and carries its own Guard: line.
# =========================================================================
...
# Written out rather than imported: the generator has no generic constant for
# it. `amd.AMD_NATIVE_WORKSPACE` holds the same string but means the AMD pod's
# volume mount, which is free to move on its own. A drift test pins the pair.
CONTAINER_WORKSPACE = "/vllm-workspace"

# step.py read_steps_from_job_dir assigns this as a bare literal, so there is
# nothing to import. Applies only under a truthy group depends_on, and only for
# the vllm-project/vllm repo. A drift test reads it out of the live source.
DEFAULT_WORKING_DIR = f"{CONTAINER_WORKSPACE}/tests"

# Keys starting with this always run: they build the images everything else
# depends on. The AMD ones have no shared prefix, so they are imported below.
# Another bare literal upstream: `_step_should_run` spells it inline as
# `step.key.startswith("image-build")`. A drift test reads the live source.
IMAGE_BUILD_KEY_PREFIX = "image-build"

# What a mirror step depends on when its override declares nothing.
# `normalize_amd_depends_on` puts this at the front of every AMD step's deps.
# Note the block-step fallback at buildkite_step.py:791 spells the same value
# as a bare literal rather than using the constant, so the two can part ways.
MIRROR_DEFAULT_DEPENDS_ON = (AMD_ARTIFACT_STEP,)

# Taken from the generator's amd.py rather than copied. Re-exported here so
# section 4 still reads as one list of what the generator does, and so callers
# keep importing every one of these facts from the same place.
AMD_ALWAYS_RUN_STEP_KEYS = _amd.AMD_ALWAYS_RUN_STEP_KEYS
AMD_NATIVE_RUNTIME_SOURCE_DEPENDENCIES = _amd.AMD_NATIVE_RUNTIME_SOURCE_DEPENDENCIES
...
# unnoticed.
# We could derive this from `step.Step.model_fields` now that the generator is
# importable, but then a field it newly declares would be instantly "known"
# while nothing here models what it does, and silently ignored instead of
# force-selected.
KNOWN_STEP_FIELDS = {
    "label",
    "key",
    ...
    "if",
    # Declared by the generator and unused by vLLM's yaml, which sets the group
    # at file level. Listed so the first step to use one is not forced for it.
    # `mount_buildkite_agent` is real plugin wiring, read in docker_plugin.py.
    "group",
...
# "exactly these, plus what they depend on". Imported, not copied.
