...
The generator's own rule takes its Step type and reads the live build config,
so this mirrors its decision on ours: the run predicate, the dependency match,
run-all, docs-only. Assumes a
...
deliberate. The pipeline is the shared INPUT, and the predicates are the generator's own
imported functions, or the one copy our selector still keeps, so a second copy here could
