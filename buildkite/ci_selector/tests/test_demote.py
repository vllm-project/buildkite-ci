...
def test_generic_literal_refused_for_leaf_routing():
    over = CONFIG_KEY_MAX_TEST_FILES + 1
...
def test_starved_member_demotion_reverted():
    g = _leafy({"generic": CONFIG_KEY_MAX_TEST_FILES + 1})
...
def test_revert_runs_before_claims_fixpoint():
    ...
    g = _leafy({"generic": CONFIG_KEY_MAX_TEST_FILES + 1, "keep": 1})
...
def test_platform_guard_literals_cover_the_filesystem_platforms(vllm_repo):
    ...
    broad edge. The set SHRINKING is the risk, because a platform-dispatched
    import would then start demoting and route by a platform word.

    Two obvious oracles are circular. Asserting the output holds none of them
    restates the filter and passes for any contents, empty included. Asking the
    job yaml reads a table whose keys are most of the set.

    The floor comes from the filesystem: a word naming both a platform module
    and a requirements file is a platform word by construction, so the
    intersection is itself the rule and no hand list is needed.

    What it does NOT catch: it floors under half the set. Dropping one of the
    rest passes here, as does a new platform word arriving without both files.
    It stops the set collapsing, not drifting."""
    platforms = {p.stem for p in (vllm_repo / "vllm/platforms").glob("*.py")}
    requirements = {p.stem for p in (vllm_repo / "requirements").glob("*.txt")}
...
def test_config_helpers_include_spec_predicates(fg, vllm_repo):
    ...
    helpers = _config_helper_literals(vllm_repo, fg.index)
...
# ---- the subtractive invariants, on the live checkout ---------------------
# These were `ci-validate dropped-edges` and the gate half of
# `ci-validate demoted-plugins`. They need nothing but a checkout -- the same
# input the session `state` fixture already builds -- so as commands they only
# ran when someone remembered the trigger. Touch `graph/demote.py` and forget,
# and nothing told you. On an already-built graph they cost ~0.3s together.


class TestSubtractiveInvariants:
    """Claims and demotions delete graph edges on purpose. Each deletion is
    sound only if the coverage it carried arrives another way, and every one of
    these fails closed."""

    def test_a_gating_literal_routes_its_member_or_was_refused_on_purpose(self, state):
        """Demotion cuts a plugin's import edge and re-routes by config word.
        A word that routes nothing, and was not refused deliberately, means the
        edge went and nothing replaced it."""
        from helpers import key_selection_gaps

        _, unrouted, _ = key_selection_gaps(state)
        assert unrouted == []

    def test_a_key_selecting_step_still_runs_when_its_member_changes(self, state):
        """Closes the loop: the steps that name a demoted member's gating word
        must actually be selected when that member changes."""
        from helpers import key_selection_gaps

        gaps, _, checked = key_selection_gaps(state)
        assert gaps == []
        # Detection floor, and the quiet one: every member sliding to
        # "skipped" leaves the invariant untested while still reporting clean.
        assert checked, "no claimed member exercised the invariant"

    def test_the_leaf_fanout_bar_never_costs_a_route(self, state):
        """The bar refuses a gating word held by many leaf files, on the theory
        it is ordinary English. That theory is half true -- it drops registered
        keys too -- so the test is not "was it a key" but "did dropping it cost
        the only route"."""
        from helpers import fanout_dropped_literals

        losses, dropped = fanout_dropped_literals(state)
        assert losses == []
        assert dropped, "the fanout bar dropped nothing; it has stopped biting"

    def test_no_demoted_plugin_is_starved(self, state):
        """A demoted plugin whose key routing found zero tests has the run-all
        fail-open as its only coverage, which is the near-run-all regression
        demotion exists to prevent."""
        from helpers import starved_members

        assert starved_members(state.full) == []
