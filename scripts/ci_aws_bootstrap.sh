...
upload_pipeline() {
    ...
    if [ ! -e ".buildkite/test-template.j2" ]; then
        curl -o .buildkite/test-template.j2 https://raw.githubusercontent.com/vllm-project/buildkite-ci/amd/mi300_slice_enablement/scripts/test-template-aws.j2?$(date +%s)
    fi
